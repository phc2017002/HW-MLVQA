...
model = Qwen2VLForConditionalGeneration.from_pretrained(
    "Qwen/Qwen2-VL-7B-Instruct-GPTQ-Int4", torch_dtype=torch.float16, device_map='auto'
)
processor = AutoProcessor.from_pretrained("Qwen/Qwen2-VL-7B-Instruct-GPTQ-Int4")

# Directory configuration
root_directory = './copied_folders_english'
output_directory = './'

output_file_name = 'output_responses_qwen2vl_7B_english_image_only_test_2.txt'
output_file_path = os.path.join(output_directory, output_file_name)

#model = model.half()

ocr_directory = './english_ocr_text_new'
    ...
    if os.path.isdir(subdir_path):
        ...
        if qa_file:
            ...
            for question in questions:
                messages = [
                    {
                        ...
                        "content": [
                        ...
                        ] + [
                            {
                                ...
                                "text": (
                                    f"{question}\n\n"
                                    "Provide the answer in a single word or phrase."
                                )
                            }
                        ],
                    }
                ...
                image_inputs = [load_and_resize_image(img_path, target_size=(768, 768)) for img_path in selected_image_paths]
                inputs = processor(
                    text=[text],
                    images=image_inputs,
                    padding=True,
...
torch.cuda.empty_cache()
gc.collect()
#  f"The extracted_text from the images is:\n{ocr_text}\n\n"